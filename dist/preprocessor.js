...
var findImportVariables = function (content) {
  ...
  rst = rst.flat(Infinity).map(function (item) {
    return item.trim();
  });
  return rst;
};
  ...
  var _loop_1 = function () {
    ...
    try {
      ...
      components = components.concat(
        findComponents(pug_uses_variables_1.findVariablesInTemplate(content)),
      );
    } catch (error) {
      // console.error(error);
    }
...
function preprocessor(content) {
  var _a = getOptions(this.query),
    includes = _a.includes,
    replace = _a.replace;
